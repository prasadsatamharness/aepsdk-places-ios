...
echo "------------------AEPPlaces-------------------"
PODSPEC_VERSION_IN_AEPPlaces=$(pod ipc spec AEPPlaces.podspec | jq '.version' | tr -d '"')
echo "Local podspec version - ${BLUE}${PODSPEC_VERSION_IN_AEPPlaces}${NC}"
SOURCE_CODE_VERSION_IN_AEPPlaces=$(cat ./AEPPlaces/Sources/PlacesConstants.swift | egrep '\s*EXTENSION_VERSION\s*=\s*\"(.*)\"' | ruby -e "puts gets.scan(/\"(.*)\"/)[0] " | tr -d '"')
